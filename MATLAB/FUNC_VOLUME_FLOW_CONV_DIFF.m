function [] = FUNC_VOLUME_FLOW_CONV_DIFF(n_SLICES,n_LOCAL,rho)
% n_SLICES: number of slices for the domain
% n_LOCAL: divisions of local Mean-Age within a slice
% rho: density for calculating the volume flow from the mass flow in kg/m^3

% preparing the variable strings for loading, saving and the diary
load_string = strcat('CLUSTERED_DATA_RE_1_SLICES_',num2str(n_SLICES),'_DELTA_',num2str(n_LOCAL),'.mat');
save_string = strcat('CLUSTERED_DATA_RE_1_VOLUME_FLOW_SLICES_',num2str(n_SLICES),'_DELTA_',num2str(n_LOCAL),'.mat');
...
load('DATA_GEOMETRIC_FLUX.mat','geometric_flux');
...
% Similar procedure for the inlet and outlet matrices. Expand the matrix
% with the geometric diffusive flux in an additional column
CPT_flow = [c0_c1_flow geometric_flux];
...
% matrix for storing the diffusive flux between two compartments
CPT1_CPT2_diff = CPT1_CPT2_flow;
...
for i = 1:n_CPT
    ...
    for j = 1:n_neighbor
        ...
        % the diffusive flux is directed from CTP1 to CPT2 and vice versa
        % with the same value. Thus, the diffusive flux over all
        % connections i to B and B to i must be considered
        
        flow_i_B_diff = sum(CPT_flow_small(array_i_B | array_B_i,4));
        
        % saving the information within CPT1_CPT2_flow and CPT1_CPT2_diff
        ...
        CPT1_CPT2_diff(row,1) = i;
        CPT1_CPT2_diff(row,2) = B;
        CPT1_CPT2_diff(row,3) = flow_i_B_diff;
        CPT1_CPT2_diff(row+1,1) = B;
        CPT1_CPT2_diff(row+1,2) = i;
        CPT1_CPT2_diff(row+1,3) = flow_i_B_diff;
        ...
        fprintf(fileID,'Mass flow from CPT %d to CPT %d [kg/s]: %d\n',B,i,flow_B_i);
        % print of diffusive fluxes optional
    end
...
% delete all rows with 0 within CPT1_CPT2_flow and _diff in the first
% column since these entries were not overwritten
CPT1_CPT2_flow(CPT1_CPT2_flow(:,1) == 0,:) = [];
CPT1_CPT2_diff(CPT1_CPT2_diff(:,1) == 0,:) = [];
...
% compartments + the inlet and outlet. Divide all entries with the desnity
% rho to convert the mass flow to a volumetric flow

% sparse(to, from, flow, n_CPT + 2, n_CPT + 2)
Flow_Matrix_conv = sparse(CPT1_CPT2_flow(:,2),CPT1_CPT2_flow(:,1),CPT1_CPT2_flow(:,3),n_CPT+2,n_CPT+2)/rho;

% create an additional matrix for the diffusive fluxes. Sp far, the entries
% are onyl geometric since no diffusion coefficient has been used yet. This
% is done at the beginning for the specific problem. Since there is no
% diffusive flux from the inlet or outlet, the last and forelast enries are
% 0
Flow_Matrix_geometric = sparse(CPT1_CPT2_diff(:,2),CPT1_CPT2_diff(:,1),CPT1_CPT2_diff(:,3),n_CPT+2,n_CPT+2);

% % the overall Flow_Matrix is the sum of the convective and the diffusive
% % flow matrix
% Flow_Matrix = Flow_Matrix_conv + Flow_Matrix_diff;
% 
...
% Flow_Normalized = Flow_Matrix_conv./Volume_all;

% Flows into the outlet
Flow_Outlet = Flow_Matrix_conv(end,1:end-2);
...
save(save_string,'c0_M1','c0_c1_flow','c0_inlet_flow','c0_outlet_flow',...
    ...
    'n_SLICES','n_LOCAL','CPT_slice','CPT_V','rho','balance',...
    ...
    'Flow_Matrix_conv','Flow_Matrix_geometric');    
